...
fn authenticate(pamh: &mut PamHandle, args: Vec<&CStr>) -> PamResultCode {
    ...
    let client = arg_map.get("client").map(String::as_str).filter(|value| !value.is_empty()).unwrap_or("pam");
...
mod tests {
    ...
    fn parses_key_value_arguments() {
        let args = vec![CString::new("client=pam").unwrap(), CString::new("ifexist=/tmp/{{.Uid}}").unwrap()];
